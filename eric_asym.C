...
void eric_asym(string FILE, Int_t HELN, Int_t DELAY, Double_t FREQ){
  ...
  H[0] = new TH1F("inc_sng_l", Form("Single Left Increments - Run %i",RUNN),  i_incrbin,  0, i_incrmax);
  H[1] = new TH1F("inc_sng_r", Form("Single Right Increments - Run %i",RUNN), i_incrbin,  0, i_incrmax);
  ...
  Int_t currLeft2 = 0;            //Same for 2nd Scaler
  Int_t currRght2 = 0;
  Int_t prevLeft2 = 0;
  Int_t prevRght2 = 0;
  ...
  for (Long64_t jentry = 0; jentry < nentries; jentry++) {
    ...
    if(nsca > 0){
      ...
      if(gdhelcyc >= 0){
        ...
        gr_bcmtm->SetPoint(scalerctr+1,jentry,(Double_t)beaminc*freq);
    	}
